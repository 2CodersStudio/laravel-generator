...
class ControllerGenerator extends BaseGenerator
{
    ...
    private function generateColumnRender($fieldTemplate, $field)
    {
        $renderText = '';

        if($field->htmlType === 'public_file'){

            $publicFileType = isset($field->htmlValues[0])?$field->htmlValues[0]:'';
            
            switch ($publicFileType) {
                case 'image':
                    $renderText = '!data? "" : "<img src=\""+data+"\" class=\"thumbnail img-responsive\"/>"';
                    break;
                
                case 'file':
                default:
                    $renderText = '"<a href=\""+data+"\" target=\"blank\" class=\"btn btn-sm btn-default" + (data?"":" disabled") + "\">Download</a>"';
                    break;
            }

        }

        $fieldTemplate = str_replace('$FIELD_RENDER$', $renderText, $fieldTemplate);
    ...
    private function generateDataTable()
    {
        ...
        foreach ($this->commandData->fields as $field) {
            ...
            $fieldTemplate = $this->generateColumnRender($fieldTemplate, $field);
            
            $headerFields[] = $fieldTemplate;
