...
class ModelGenerator extends BaseGenerator
{
    ...
    private function fillTemplate($templateData)
    {
        ...
        $templateData = $this->generateAccessorsAndMutators($templateData);
    ...
    private function generatePublicFileAccesors($field)
    {
        $accessorsText = infy_nl_tab(1, 1).'public function get'.ucfirst(camel_case($field->name)).'Attribute($value){';
        $accessorsText .= infy_nl_tab(1, 2).'if(empty($value)) return $value;';
        $accessorsText .= infy_nl_tab(1, 2).'return asset(\'storage/\'.$value);';
        $accessorsText .= infy_nl_tab(1, 1).'}';
        return $accessorsText;
    }
    
    private function generatePublicFileMutators($field)
    {
        $folder = isset($field->htmlValues[1])? $field->htmlValues[1] : '/';

        $mutatorsText = infy_nl_tab(1, 1).'public function set'.ucfirst(camel_case($field->name)).'Attribute($value){';
        $mutatorsText .= infy_nl_tab(1, 2).'$this->attributes[\''.$field->name.'\'] = $value->store(\''.$folder.'\',\'public\');';
        $mutatorsText .= infy_nl_tab(1, 1).'}';
        return $mutatorsText;
    }

    public function generateAccessorsAndMutators($templateData)
    {
        $accessors = [];
        $mutators = [];
        ...
        foreach ($this->commandData->fields as $field) {
            ...
            switch ($field->htmlType) {
                case 'public_file':
                    $accessors[] = $this->generatePublicFileAccesors($field);
                    $mutators[] = $this->generatePublicFileMutators($field);
                    break;
        ...
        $templateData = str_replace('$ACCESSORS$', implode(infy_nl_tab(1, 1), $accessors), $templateData);

        $templateData = str_replace('$MUTATORS$', implode(infy_nl_tab(1, 1), $mutators), $templateData);
